...
async def start(
    ...
    if await user_service.is_user_exists(message.from_id):
        await message.answer(
            "Уважаемые коллеги, данный бот разработан для уведомления о показателях № 2 «Мобилизационная база» и индикатора № 3 «Вовлеченность депутатов ЛДПР в проведение ВПГ», показателя № 1 «Выполнение мероприятий планирования» светофора"
        )
        return
